...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === "GET") {
      if (id) {
        const rows = await getDb().select().from(agenti).where(and(eq(agenti.orgId, orgId), eq(agenti.id, parseInt(id))));
        return res.json(rows[0] || null);
      }
      const rows = await getDb().select().from(agenti).where(eq(agenti.orgId, orgId));
      return res.json(rows);
    }

    if (req.method === "POST") {
      const body = await parseBody(req);
      const result = await getDb().insert(agenti).values({ ...body, userId, orgId, orgShortId, createdByName: userName }).returning();
    ...
    if (req.method === "PUT") {
      ...
      const [row] = await getDb().update(agenti).set({ ...data, updatedByName: userName }).where(and(eq(agenti.orgId, orgId), eq(agenti.id, parseInt(rowId)))).returning();
      return res.json(row);
    }
    ...
    if (req.method === "DELETE") {
      ...
      await getDb().delete(agenti).where(and(eq(agenti.orgId, orgId), eq(agenti.id, parseInt(deleteId))));
